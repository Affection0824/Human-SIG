...
- Each benchmark/category folder contains: `input.txt` (required, file extension is `.txt`, but content format varies by method), `scraper.py` (optional), and `data.csv`.
...
- If `data.csv` does not exist, check if `scraper.py` exists:
  ...
  - If `scraper.py` does not exist but `input.txt` exists, create `scraper.py` based on `scraping_method` and execute it.
...
2. For each entry:
   ...
   - Check file status in the benchmark/category directory:
     ...
     - **If both `data.csv` and `scraper.py` do not exist, but `input.txt` exists:** Create `scraper.py` based on `scraping_method`, then execute it to generate `data.csv`. All input files are `input.txt` format (content may be HTML or JSON format text, depending on the method).
     - **If `input.txt` does not exist:** Skip this entry (do not interrupt the workflow).
...
- **Input files:** 
  - `artificial_analysis/input.txt`: User-manually copied `<table>` element HTML (unified table, shared by all benchmarks). File extension is `.txt`, but content is HTML format text.
  - `{benchmark_name}/input.txt`: Optional, benchmark description or reference file.
- **Script tasks:**
  - Read `../input.txt` (unified table file in parent directory, HTML format text).
  - Parse the HTML text directly from the file content (use `pandas.read_html()` with StringIO or pass the text string directly).
...
- **Parsing requirement:** Must parse HTML directly from the text content of `input.txt` file, not from a file path.
...
- **Directory:** `Human-SIG/data/raw/frontiermath/{subdirectory}/`
  - `frontiermath_tier_1_3/`
  - `frontiermath_tier4/`
- **Input file:** `input.txt` in each subdirectory: User-manually copied `<table>` element HTML. File extension is `.txt`, but content is HTML format text.
- **Script tasks:**
  - Read `input.txt` in the subdirectory (HTML format text).
  - Parse HTML table directly from the text content (use BeautifulSoup or `pandas.read_html()` with StringIO).
...
- **Input file:** `input.txt`: User-manually copied JSON element (from webpage source code, JSON format text, may contain irrelevant information before and after).
- **Script tasks:**
  - Read `input.txt` file (JSON format text).
  - Handle possible irrelevant information before and after (extract valid JSON data from the text).
  - Parse JSON structure directly from the text content and extract model names and scores.
...
- **Parsing requirement:** Must parse JSON directly from the text content of `input.txt` file, not from a file path.
...
- **Input file:** `input.txt` in each category subdirectory: User-manually copied `<table>` element HTML. File extension is `.txt`, but content is HTML format text.
- **Script tasks:**
  - Read `input.txt` in the category subdirectory (HTML format text).
  - Parse the HTML table directly from the text content (use `pandas.read_html()` with StringIO or pass the text string directly).
