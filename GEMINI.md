...
│      ├── model_extraction/   # LMArena model information (auto-generated by step 3.2.2)
...
│      │       ├── cleaned_data.csv    # Model names, scores, and ranks (Step 3.2.1 output)
│      │       └── mapping.json        # Model name to LMArena model ID mapping (Step 3.2.4 output)
│      ├── review_files/       # Review files ready for human approval
│      │   └── {benchmark_id}_review.json  # Auto-reviewed files (Step 3.2.2 output)
│      ├── normalized_scores/
│      └── master_table/
├── src/
...
- **4.3.2b:** Test Execution: H5 (Variance) - The "Task Type Interaction" Test.
  ...
  - **Statistical Test:** 
    - For each group, compute Spearman correlation between CV and construct validity with Perceived Utility, along with 95% bootstrap confidence intervals (5000 iterations).
