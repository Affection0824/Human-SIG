...
- **Scraping Protocol:**
  ...
  - **Directory Structure:** Each benchmark has its own folder structure following the README.md organization scheme:
    ...
    - Each benchmark folder contains: `input.txt` (or `input.html`), `scraper.py` (if applicable), and `{name}_data.csv` (or `{name}_data.json` for VALS.ai).
    - **Important:** All `input.txt` files and `scraper.py` scripts for benchmarks with data already exist in their respective folders. DO NOT create new scripts; use the existing ones.
  - **LMArena Data Acquisition:** LMArena data is pre-prepared and available in `Human-SIG/data/raw/lmarena_leaderboard/filtered_elo_dump.json`. This file contains filtered and consolidated ELO scores for all LMArena categories (Overall, Coding, Math, Hard Prompts, Creative Writing, Instruction Following, Expert). The agent should load this file directly without performing any scraping.
  - **Output Format:** All scraped data must be saved as CSV files (not JSON, except for VALS.ai which may use JSON initially). The CSV files should contain at minimum: model_name column and score column.
...
- **1.1.1:** Create a `.gitignore` in `Human-SIG/` excluding `__pycache__`, `*.DS_Store`, and `*.log` (if it doesn't already exist).
...
- **Need to Create (if not exist):**
  ...
  - Missing scripts in `src/ingestion/`, `src/processing/`, `src/analysis/`
  - `config/mapping.json` (if it doesn't exist)

**Directory Structure:** The following structure follows the organization scheme defined in `README.md`, where each benchmark has its own folder containing `input.txt`, `scraper.py` (if applicable), and `{name}_data.csv`. The structure separates benchmarks with existing data from those without data, and further categorizes them by scraping method. **Note:** Scraping scripts (`scraper.py`) are located in the benchmark folders under `data/raw/`, NOT in `src/ingestion/benchmarks/`. The `src/ingestion/benchmarks/` directory may contain legacy scripts but should not be used.
...
│  │  │   │       ├── input.txt         # URL or data description file
...
│  │  # Note: LMArena data is pre-prepared and available in data/raw/lmarena_leaderboard/filtered_elo_dump.json
│  │  # Note: Individual scraping scripts are located in data/raw/benchmarks_*/{method}/{benchmark_name}/scraper.py
│  │  # The src/ingestion/benchmarks/ directory may contain legacy scripts but should not be used
...
- `input.txt` for manual benchmarks (manual, manual_source_code, artificial_analysis, vals_ai_manual_json) contains URL or data acquisition instructions.
...
- **1.4.1: Verify existence of `Human-SIG/config/metadata.json`.**
...
- **1.4.2: Create `Human-SIG/config/mapping.json` (if it doesn't already exist).**
...
- **2.2.1:** Execute existing scraping scripts or handle data files for each benchmark.
  ...
  - **Directory Structure:** The data files are organized according to the README.md structure:
    ...
    - **Script Location:** All scraping scripts (`scraper.py`) are located directly in the benchmark folders under `data/raw/benchmarks_*/{method}/{benchmark_name}/scraper.py`. The `src/ingestion/benchmarks/` directory may contain legacy scripts but should NOT be used.
  ...
  - **Scraping Strategy for Each Benchmark:**
    ...
    6. **Method E: Manual (scraping_method == "manual"):**
       ...
       - **Implementation:**
         - **File Status:** For `manual_direct`, all files (`input.txt`, `{name}_data.csv`) already exist.
    ...
    7. **Method F: Manual Source Code (scraping_method == "manual_source_code"):**
